...
export const deleteMovie=createAsyncThunk("movie/delete",async(payload)=>{
   try {
    const {data}=await axios.delete(`http://localhost:5000/api/movies/${payload}`);
    return data
...
export const updateMovie=createAsyncThunk("movie/update",async({id,newMovie})=>{
    console.log(id,newMovie)
    try {
        const {data}=await axios.put(`http://localhost:5000/api/movies/${id}`,newMovie)
        return data;
